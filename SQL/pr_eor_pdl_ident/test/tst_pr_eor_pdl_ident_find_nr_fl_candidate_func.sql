...
-- Функция: eor.pr_eor_pdl_ident_find_nr_fl_candidate_func(TEXT, DATE)
--
-- Сценарии:
...
BEGIN
    ...
    IF l_nr_fl < 6 OR l_master < 6 THEN
        RAISE EXCEPTION 'Тестовые данные не подготовлены: nr_fl=%, master=%', l_nr_fl, l_master;
    END IF;

    RAISE NOTICE 'Предварительная проверка пройдена: nr_fl=%, master=%', l_nr_fl, l_master;
END $$;
...
BEGIN
    RAISE NOTICE '===== СТАРТ ТЕСТИРОВАНИЯ =====';
    ...
    l_name := 'Т1: ФИО + ДР';
    BEGIN
        l_find_id := eor.pr_eor_pdl_ident_find_nr_fl_candidate_func(
    ...
    BEGIN
        ...
        -- 5.3. Очистка (выполняется всегда, даже при FAIL)
        DELETE FROM eor.idw_mr_master      WHERE etalon_registry_id = 9900001;
        DELETE FROM eor.idwh2_etalon_nr_fl WHERE etalon_registry_id = 9900001;
    ...
    BEGIN
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        -- Гарантированный возврат состояния
        BEGIN
            UPDATE eor.idw_mr_master
               SET deleted_sign = '0'
             WHERE etalon_registry_id = 2830234;
    ...
    BEGIN
        ...
        -- 10.2. Второй вызов (перезаписывает tmp)
        PERFORM eor.pr_eor_pdl_ident_find_nr_fl_candidate_func(
            'МИРЗОЕВ ФЕЙРУЗ МУРАДОВИЧ', '1989-11-25'::date);

        -- 10.3. Проверяем, что записи 1-го вызова исчезли
...
BEGIN
    ...
    DELETE FROM eor.idwh2_etalon_nr_fl WHERE etalon_registry_id = 9900001;
    GET DIAGNOSTICS l_del_nr_fl = ROW_COUNT;
    IF l_del_master > 0 OR l_del_nr_fl > 0 THEN
        RAISE NOTICE 'Финал: удалён дубликат Т5 (master=%, nr_fl=%)', l_del_master, l_del_nr_fl;
    END IF;
    ...
    GET DIAGNOSTICS l_del_nr_fl = ROW_COUNT;
    IF l_del_master > 0 OR l_del_nr_fl > 0 THEN
        RAISE NOTICE 'Финал: удалён массовый набор Т9 (master=%, nr_fl=%)', l_del_master, l_del_nr_fl;
    END IF;
...
BEGIN
    SELECT
      ...
      + (SELECT COUNT(*) FROM eor.idwh2_etalon_nr_fl
          WHERE etalon_registry_id = 9900001
             OR etalon_registry_id BETWEEN 9800001 AND 9801001)
      INTO l_leftover;
